...
public class GlideCatchUtil {
    ...
    public static GlideCatchUtil getInstanceContext() {
        if (null == instance) {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(BaseApplication.getInstanceContext().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(BaseApplication.getInstanceContext().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR, true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(BaseApplication.getInstanceContext()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(BaseApplication.getInstanceContext()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(BaseApplication.getInstanceContext()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
